...
#include "..\interface\master\tlm_master_bus_if.h"
#include "..\interface\slave\tlm_slave_bus_if.h"
...
struct tlm_bus : public sc_prim_channel, public tlm_master_bus_if, public tlm_slave_bus_if {
	...
	private: 
		...
		sc_event w2slave_ack_ev; // Write to Slave acknowledge
		...
		tlm_bus(const tlm_bus&) {} // Copy constructor in private
		                           // So that it won't be generated
		                           //  in public
	...
	public:
		...
		const sc_event& int0_event() const;               // Interrupt0 event implementation
		const sc_event& int1_event() const;               // Interrupt1 event implementation
		const sc_event& rw_event() const;                 // Read-Write event from Master implementation
		void w2slave_ack();                               // Write data to Slave ack implementation
